...
def plot_spacecraft_mission(
    plan: Plan, sim_result: SimResult, output_dir: Path, mission_name: str = "Spacecraft Mission"
) -> Dict[str, Path]:
    """
    ...
    for i, (label, etype, t_start, duration) in enumerate(
        zip(event_labels, event_types, start_times, durations)
    ):
    ...
    ax3.bar(
        ...
        width,
        label="Observations",
        color=color_map.get("observation", "#FF6B6B"),
        edgecolor="black",
    )
    ax3.bar(
        days + width / 2,
        ...
        width,
        label="Downlinks",
        color=color_map.get("downlink", "#4ECDC4"),
        edgecolor="black",
    )
