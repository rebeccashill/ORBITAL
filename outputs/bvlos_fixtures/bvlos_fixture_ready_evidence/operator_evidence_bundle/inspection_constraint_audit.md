...
| Top limiting constraint | Wind / weather margin, PASS, margin 3.6 m/s | Start the detailed review here before changing or releasing the mission. |
...
- Selection detail: Wind / weather margin was selected because it had the highest risk_points value (12.0) among 5 audited constraint groups. Its margin was 3.6 m/s against a warning threshold of 2.0 m/s.
...
| Geofence / no-fly-zone clearance | PASS | 254.1 m | Checks whether the route remains outside no-fly zones and preserves the configured stand-off buffer. | Geofence clearance protects people, assets, restricted areas, and customer boundaries when navigation or wind uncertainty appears. | Keep the geofence file and route review in the evidence bundle, then confirm site boundaries before flight. |
...
| Turn / bank feasibility | PASS | 0.166 rad/s | Checks whether planned turns stay within configured bank-angle and turn-rate capability. | Overly aggressive turns can break route tracking, increase energy use, and reduce safety margins near assets or geofences. | Keep the planned speed and turn assumptions, then verify they match the aircraft operating envelope. |
...
| final simulated battery | 834.3811805568919 | Wh | simulation.scalars.final_battery_Wh |
| energy used | 15.618819443108123 | Wh | simulation.scalars.energy_used_Wh |
...
| minimum simulated clearance | 329.1022607761889 | m | simulation.scalars.geofence_min_clearance_m |
...
| planned cruise speed | 22.37060015452591 | m/s | plan.metadata.cruise_speed_mps |
...
| Input | Value | Unit | Source |
| --- | ---: | --- | --- |
| robustness cases configured | 1 | cases | robustness.cases |
| robustness cases run | 1 | cases | robustness summary |
...
- Scenario path: examples/bvlos_fixtures/bvlos_ready_evidence_fixture.yaml
- Seed: 11
- Iterations: 8
- Restarts: 1
- Robustness cases configured: 1
- Robustness cases run: 1
- Command: python -m mission_framework.cli examples\bvlos_fixtures\bvlos_ready_evidence_fixture.yaml --generated-at 2026-09-15T05:24:50Z --outdir outputs\bvlos_fixtures
...
- Battery reserve margin: PASS, margin 134.4 Wh, risk points 10.8
...
- Observed evidence: final battery Wh: 834.4; required reserve Wh: 700.0
...
- Observed evidence: required clearance m: 75.0; min clearance m: 329.1; geofence violated: no
