...
| --- | --- | --- |
| Mission status | MODIFY | First feasibility read from the modeled constraints. |
| Mission risk | HIGH | Higher risk means the operator should spend more time on the top drivers. |
| Top limiting constraint | Battery reserve margin, FAIL, margin -41.0 Wh | Start the detailed review here before changing or releasing the mission. |
...
- Constraint: Battery reserve margin
- Status: FAIL
- Margin: -41.0 Wh
- Plain-English read: Battery reserve margin is failing in the modeled mission; modify the plan before treating this sortie as release-ready.
- Why this matters: A BVLOS inspection needs enough remaining energy for delay, diversion, recovery, and conservative abort decisions.
- Operator action: Do not fly as modeled; shorten the mission, add a relaunch / battery swap, or use a higher-endurance aircraft.
- Selection detail: Battery reserve margin was selected because it had the highest risk_points value (89.3) among 5 audited constraint groups. Its margin was -41.0 Wh against a warning threshold of 88.0 Wh.
...
| --- | --- | --- |
| Energy / battery reserve | Checks whether the planned sortie lands with the operator-required battery reserve still available. | Do not fly as modeled; shorten the mission, add a relaunch / battery swap, or use a higher-endurance aircraft. |
...
| --- | --- | ---: | --- | --- | --- |
| Energy / battery reserve | FAIL | -41.0 Wh | Checks whether the planned sortie lands with the operator-required battery reserve still available. | A BVLOS inspection needs enough remaining energy for delay, diversion, recovery, and conservative abort decisions. | Do not fly as modeled; shorten the mission, add a relaunch / battery swap, or use a higher-endurance aircraft. |
| Weather / wind margin | PASS | 3.6 m/s | Checks whether modeled or forecast wind stays below the configured safe operating limit. | Wind reduces endurance, increases tracking error, and can turn a feasible route into a recovery or containment problem. | Confirm launch-time weather against operator minimums and keep the forecast source with the mission package. |
| Geofence / no-fly-zone clearance | PASS | 273.1 m | Checks whether the route remains outside no-fly zones and preserves the configured stand-off buffer. | Geofence clearance protects people, assets, restricted areas, and customer boundaries when navigation or wind uncertainty appears. | Keep the geofence file and route review in the evidence bundle, then confirm site boundaries before flight. |
...
| Turn / bank feasibility | PASS | 0.252 rad/s | Checks whether planned turns stay within configured bank-angle and turn-rate capability. | Overly aggressive turns can break route tracking, increase energy use, and reduce safety margins near assets or geofences. | Keep the planned speed and turn assumptions, then verify they match the aircraft operating envelope. |
...
| required battery reserve | 880.0 | Wh | vehicle.battery_reserve_Wh |
| final simulated battery | 838.9704116169329 | Wh | simulation.scalars.final_battery_Wh |
| energy used | 11.029588383067107 | Wh | simulation.scalars.energy_used_Wh |
...
| weather wind gust | 5.4 | m/s | weather.resolved.wind_gust_mps |
| weather source | Operator-packaged field weather observation |  | weather.resolved.source |
| weather timestamp | 2026-09-15T05:15:00Z | UTC | weather.resolved.timestamp_utc |
...
| minimum simulated clearance | 348.13569442093285 | m | simulation.scalars.geofence_min_clearance_m |
...
| planned cruise speed | 12.0 | m/s | plan.metadata.cruise_speed_mps |
...
- Scenario path: examples/bvlos_fixtures/bvlos_modify_no_go_fixture.yaml
- Seed: 14
- Iterations: 8
- Restarts: 1
- Robustness cases configured: 0
- Robustness cases run: 0
- Command: python -m mission_framework.cli examples\bvlos_fixtures\bvlos_modify_no_go_fixture.yaml --generated-at 2026-09-15T05:24:50Z --outdir outputs\bvlos_fixtures
...
- Location: Palo Alto utility corridor fixture
- Forecast window start: 2026-09-15T04:45:00Z
- Forecast window hours: 2.0
- Wind speed: 4.2 m/s
- Wind direction: 282.0 deg
- Wind gust: 5.4 m/s
...
- Battery reserve margin: FAIL, margin -41.0 Wh, risk points 89.3
- Wind / weather margin: PASS, margin 3.6 m/s, risk points 12.0
...
- Status: FAIL
- Status meaning: Modeled margin is negative; modify the mission before release.
- Margin: -41.0 Wh
- Warning margin: 88.0 Wh
- Observed evidence: final battery Wh: 839.0; required reserve Wh: 880.0
- Recommended operator action: Do not fly as modeled; shorten the mission, add a relaunch / battery swap, or use a higher-endurance aircraft.
...
- Observed evidence: max horizontal wind mps: 5.4; weather audit wind mps: 5.4; max safe wind mps: 9.0; weather source: Operator-packaged field weather observation; weather timestamp utc: 2026-09-15T05:15:00Z; 7 more fields in the JSON audit
...
- Observed evidence: required clearance m: 75.0; min clearance m: 348.1; geofence violated: no
